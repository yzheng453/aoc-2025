fn parse_input(input_str: &String) -> impl Iterator<Item = i32> {
    input_str.split_whitespace().map(|line| {
        let direction = &line
            .chars()
            .nth(0)
...
fn part1(input_str: String) -> i32 {
    let (_, count) = parse_input(&input_str).fold((50, 0), |(position, count), rotation| {
